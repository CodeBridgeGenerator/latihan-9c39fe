...
const KategoriDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate0 = (rowData, { rowIndex }) => <p >{rowData.kategori}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "kategori"});
...
<Column field="kategori" header="kategori" body={pTemplate0} filter={selectedFilterFields.includes("kategori")} hidden={selectedHideFields?.includes("kategori")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload Kategori Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
      ...
      <Dialog header="Search Kategori" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default KategoriDataTable;